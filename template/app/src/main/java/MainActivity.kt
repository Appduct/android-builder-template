...
class MainActivity : AppCompatActivity() {
    ...
    private fun injectBiometricLoginShim(view: WebView?) {
        ...
        val js = """
            ...
            function isBioEl(el){try{if(!el||el.nodeType!==1)return false;var explicit=el.matches&&el.matches('[data-biometric-login],#biometric-login,.biometric-login,[data-biometric]');if(explicit)return true;var tag=(el.tagName||'').toLowerCase();var type=((el.getAttribute&&el.getAttribute('type'))||'').toLowerCase();var interactive=tag==='button'||tag==='a'||el.getAttribute('role')==='button'||(tag==='input'&&(type==='button'||type==='submit'));if(!interactive)return false;var role=(el.getAttribute&&(el.getAttribute('aria-label')||''))||'';var txt=((el.innerText||el.textContent||'')+' '+role+' '+(el.title||'')+' '+(el.value||'')).trim();if(!txt||txt.length>80)return false;return BIO_RE.test(txt);}catch(e){return false;}}
            function findBio(target){try{if(!target||!target.closest)return null;var el=target.closest('button,a,[role=button],input[type=button],input[type=submit],[data-biometric-login],#biometric-login,.biometric-login,[data-biometric]');return isBioEl(el)?el:null;}catch(e){}return null;}
